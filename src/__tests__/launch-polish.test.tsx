...
describe("FIX 1 — roiFraming helper", () => {
  it("sub-month: returns a humble line, never '0 months'", () => {
    ...
    expect(roiFraming(50)).toBe("less than 1 month of Quote Reclaim");
  ...
  it("exact spec examples: 948, 1580, 4000, 12000", () => {
    expect(roiFraming(948)).toBe("12 months of Quote Reclaim");
    expect(roiFraming(1580)).toBe("20 months of Quote Reclaim");
    expect(roiFraming(4000)).toBe("4x a full year of Quote Reclaim");
    expect(roiFraming(12000)).toBe("12x a full year of Quote Reclaim");
  });
  ...
  it("roiPieces returns the structured shape callers can compose around", () => {
    expect(roiPieces(50)).toEqual({ kind: "subMonth" });
    expect(roiPieces(1580)).toEqual({ kind: "months", months: 20 });
