...
describe("unified recovery message library", () => {
  ...
  it("uses the exact six centralized messages for fallback schedules", () => {
  ...
  it("keeps the required shame-removal closes verbatim", () => {
    expect(
      getRecommendedMessage("Scope Rescue", {
        firstName: "Jane",
        trade: "Concrete",
      }),
    ).toContain("If it's a pass, 'no' works too");
    expect(
      getRecommendedMessage("Clean Closeout", {
        firstName: "Jane",
        trade: "Concrete",
      }),
    ).toContain("no re-quote needed");
    expect(
      getReplyPlaybook("Concrete").find(
        (path) => path.id === "no_response_breakup",
      )?.response,
